...
export async function POST(request: NextRequest) {
  ...
  try {
      ...
      Statement: [
        {
          ...
          Condition: {
            StringLike: {
              "s3:prefix": [`${session.user.id}/*`, `${session.user.id}`],
            },
          },
        },
        {
          ...
          Resource: [`arn:aws:s3:::${BUCKET_NAME}/${session.user.id}/*`],
        },
      ],
    };
    ...
    const assumeRoleCommand = new AssumeRoleCommand({
      RoleArn: ROLE_ARN,
      RoleSessionName: `s3-upload-${session.user.id}-${Date.now()}`,
...
export async function GET() {
  return NextResponse.json({
    ...
    method: "POST",
    description:
      "Send a POST request to get temporary AWS credentials (requires authentication)",
  });
}
